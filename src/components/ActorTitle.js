...
const ActorTitle = ({ data, actor }) => {
  const getActorTitles = () => {
    const actorData = data.filter((show) =>
      Object.values(show.cast).includes(actor),
    );
    const totalPresence = actorData.length;
    const castCount = actorData.filter((show) =>
      Object.keys(show.cast).includes(actor),
    ).length;
    const servingTime = actorData.filter((show) =>
      Object.values(show.serving).includes(actor),
    ).length;
    //TODO update the title logic
    const titles = [
